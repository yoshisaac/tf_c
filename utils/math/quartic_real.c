...
int solve_real_poly(int degree, const double *poly, double *results)
{
    ...
    for (i = 0; i < degree; ++i) normalized_poly[i] = poly[i] / a;
...
static void calc_shifted_coefs(double shift, int degree, const double *src, double *dst)
{
    ...
    int dst_i, src_i;
    for (dst_i = 0; dst_i <= degree; ++dst_i) dst[dst_i] = 0;
    ...
    for (src_i = 0; src_i <= degree; ++src_i)
        for (dst_i = 0; dst_i <= src_i; ++dst_i) dst[dst_i] += src[src_i] * shift_powers[src_i - dst_i] * binomials[src_i][dst_i];
}
